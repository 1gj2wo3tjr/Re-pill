            ...
            <div style={{ border: "1px solid black", height: "10rem", marginTop: "5%", display: "flex", alignItems: "center", borderRadius: "10px" }} key={item.id}>
              ...
              <div style={{ width: "25%", display: "flex", flexDirection: "column", marginLeft: "5%" }}>
                ...
                {item.order_status === 1 ? (
                    ...
                    <button className={styles.order_cancle_order_button_mob} onClick={() => alert("취소대기중입니다.")}>취소완료</button>
                  )}
              </div>
            </div>)}
            <ReviewRegisterModal open={openRegister} setOpen={setOpenRegister} id={productId} />
            {openDetail && <ReviewDetailModal open={openDetail} setOpen={setOpenDetail} reviewId={reviewId} productId={productId} title={title} imgUrl={imgUrl} />}
        </Container>
      </>) : (
      <>
        <Container>
            ...
            <div style={{ border: "1px solid black", height: "15rem", marginTop: "5%", display: "flex", alignItems: "center", borderRadius: "10px" }} key={item.id}>
              ...
              <div style={{ width: "20%", display: "flex", flexDirection: "column", marginLeft: "10%" }}>
                ...
                {item.order_status === 1 ? (
                    ...
                    <button className={styles.order_cancle_order_button} onClick={() => alert("취소대기중입니다.")}>취소완료</button>
                  )}
              </div>
            </div>)}
            <ReviewRegisterModal open={openRegister} setOpen={setOpenRegister} id={productId} title={title} imgUrl={imgUrl} />
            {openDetail && <ReviewDetailModal open={openDetail} setOpen={setOpenDetail} reviewId={reviewId} productId={productId} title={title} imgUrl={imgUrl} />}
        </Container>
      </> 
      )}
