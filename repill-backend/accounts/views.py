...
class KakaoLogin(View):
    def get(self, request):
        kakao_access_token = request.headers["Authorization"]  # frontend에서 받은 토큰
        headers = ({"Authorization": f"Bearer {kakao_access_token}"})
        url = "https://kapi.kakao.com/v1/user/me"  # 회원 정보 확인을 위한 카카오 API 주소
        social_user = requests.request("POST", url, headers=headers).json()

        if User.objects.filter(uid = social_user["id"]).exists():  # DB에 회원정보 있으면 로그인
